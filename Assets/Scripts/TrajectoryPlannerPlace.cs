...
public class TrajectoryPlannerPlace : MonoBehaviour
{
    // Hardcoded variables
    ...
    [SerializeField]
    GameObject m_Place;   // Place GameObject for place pose

    // Assures that the gripper is always positioned above the m_Place cube before placing.
    readonly Vector3 m_PlacePoseOffset = Vector3.up * 0.1f; // Offset for the place pose
    ...
    void Start()
    {
        ...
        {
            linkName += SourceDestinationPublisher1.LinkNames[i];
    ...
    /// <summary>
    ///     Send the place_pose to the ROS service and execute the returned trajectory.
    /// </summary>
    public void PublishJoints()
    {
        ...
        // Calculate the place position relative to the robot's base frame
        Vector3 placePositionRelative = m_Place.transform.position - m_UR3e.transform.position;

        // Apply offset and convert to ROS FLU coordinate system
        request.pick_pose = new PoseMsg
        {
            position = (placePositionRelative + m_PlacePoseOffset).To<FLU>(), // Convert to ROS FLU
        ...
        Debug.Log($"Place Pose Position (ROS FLU): {request.pick_pose.position}");
        Debug.Log($"Place Pose Orientation (ROS FLU): {request.pick_pose.orientation}");
        ...
        if (response.trajectories != null)
        {
            ...
            foreach (var t in response.trajectories[0].joint_trajectory.points)
            {
                ...
                for (var joint = 0; joint < m_JointArticulationBodies.Length; joint++)
                {
                    ...
                    joint1XDrive.target = result[joint]; // Corrected: Set target position
